...
name = "oauth2_proxy-v#{node["oauth2_proxy"]['version']}.linux-amd64.tar.gz"
...
execute 'install_oauth2_proxy' do
  command "tar xf #{name};mv oauth2_proxy-v#{node["oauth2_proxy"]['version']}.linux-amd64/oauth2_proxy /usr/bin/oauth2_proxy"
  cwd c
  action :nothing
end
